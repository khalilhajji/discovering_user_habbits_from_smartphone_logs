'''
This file contains utils operations that are done on the clean version of the data. To have more information on how this dta is built, see documentation file in doc directory
'''
#!/usr/bin/env python
import sys
import pprint as pp
...
'''
DataExtractor is the class that handles all the data extraction operations of the cleaned version of the data
'''
class DataExtractor:
	test_user_id = 7
	...
	json_data_dir = "/speech/dbwork/mul/students/dehajjik/clean_data/"
	...
	#the different features available in the dataset
	features = ['activityRecognitionResult_main',
	'androidActivityRecognitionResult_main',
	'appLaunch',
	'battery_main',
	'bluetooth',
	'event',
...
class DataOperations:
	...
	'''
	date is a string in the form %Y-%m-%d %H:%M:%S
	millis is a float representing the unix time in milliseconds
	
	construct an interval where the start date is the same than the end date:
	date , millis->date , millis
	'''
	@staticmethod
	def instanciate_interval(date, millis):
		complete_date = date+" , "+str(millis)
		return DataOperations.build_interval(complete_date, complete_date)
