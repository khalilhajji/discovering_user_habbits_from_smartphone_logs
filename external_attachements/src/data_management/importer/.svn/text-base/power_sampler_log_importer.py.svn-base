...
class PowerSamplerLogImporter(Importer):
    """
    Download power sampler log from S3
    """
    ...
    PW_INFO = "pwpredict_info"
    RUN_APPS = "run_apps"
    SPEC = "spec"
    
    file_filters = [PW_INFO, RUN_APPS, SPEC]
    ...
    def __init__(self, raw_log_path = raw_log_path, json_usrs_log_path = json_usrs_log_path, conn = conn, bucket_filters = bucket_filters, file_filters = file_filters):
        ...
        super(PowerSamplerLogImporter, self).__init__(raw_log_path, json_usrs_log_path, conn, bucket_filters, file_filters)
        
        
        logger = logging.getLogger("PowerSamplerLogImporter")
    ...
    def _save_as_json(self, key, raw_log_path, json_usrs_log_path):
        ...
        guid = elms[self.user_idx] # guid generated by UUID.random()
        ...
        dst_filename = "%s.json" % src_filename.split(".%s" % self.GZ_EXTENSION)[0]
    ...
    def _get_keys(self, all_keys, file_filters = []):
        ...
        for file_filter in file_filters:
            file_filter_regexes.append(re.compile("^.*%s.*\.%s$" % (file_filter, self.GZ_EXTENSION)))
            pass
...
def main():
    importer = PowerSamplerLogImporter()
    importer.imports()
