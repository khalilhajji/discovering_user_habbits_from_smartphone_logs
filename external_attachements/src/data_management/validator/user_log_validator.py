...
class UserLogValidator(Validator):
    ...
    now 1 OR 3. (keep valid users who are no longer collecting data)
    """
    ...
    users_log_path = "/speech/dbwork/mul/reco1/AppPrediction/SonyLogging/Logs/from_TKY/pulled_from_TKY/mixs_logs/json/usrs/*/all/all_in_one_log.json"
    #users_log_path = "/speech/dbwork/mul/reco1/AppPrediction/SonyLogging/Logs/from_TKY/pulled_from_TKY/mixs_launcher_logs/mixs_launcher_logs/json/usrs/*/all/all_in_one_log.json"
    ...
    # user black list
    user_black_list = {
        "351680064713724": 1, # eutec dev-purpose
        "358938040438995": 1, # had not userd actually for daily use.
        "352136065015162": 1, # isd dev-purpose
        "355243050110276": 1, # just in case
        "357931057103710": 1, # my own smt contracted with a carrer, not daily usage
        "357662050800388": 1, # had not used actually for daily use.
        "352136065012607": 1, # had not used actually for daily use.
        "358240050409531": 1, # had not used actually for daily use.
        "351680069788382": 1, # had not used actually for daily use.
    }
    ...
    def _validate(self, ):
        ...
        for path in log_paths:
            ...
            if self.validate_user(data):
                user_id = self._extract_user(data["baseInfo"])
    ...
    def validate_user(self, data):
        ...
        Arguments:
        - `data`: time-ascending order json log
        """
        ...
        # logical OR operatoin for conditons
        self.logger.debug("flag1:%d, flag3:%d", bin_percent_flag, total_n_logs_flag)
        if bin_percent_flag or total_n_logs_flag:
            return True
