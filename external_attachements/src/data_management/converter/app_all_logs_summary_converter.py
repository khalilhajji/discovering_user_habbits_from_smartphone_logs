...
class AppAllLogsSummaryConverter(AllLogsConverter):
    """
    Convert a converted file by AppAllLogsConverter, or all_in_one_json_log to time line js format.
    """

    # base direcotry and pathes
    all_in_one_json_log_path = "/data/mixs_logs/%s/apps/*/%s/%s" % (AllLogsConverter.json_log_dir, AllLogsConverter.all_in_one_log_dir, AllLogsConverter.all_in_one_json_file)
    ...
    def __init__(self, ):
        ...
        logger = logging.getLogger("AppAllLogsSummaryConverter")
    ...
    def _get_all_in_one_log_paths(self, all_in_one_json_log_path = all_in_one_json_log_path):
        """
        Arguments:
        - `all_in_one_json_log_path`: all-in-one json file for a app
    ...
    def _convert(self, all_in_one_log_path):
        ...
        self._add_app_name(tljs_obj, all_in_one_json_log)        
    ...
    def _add_log_info(self, tljs_obj, log_info_t, t):
        ...
        # baseInfo and location
        tljs_obj[self.LOG_INFO][t][self.BASE_INFO] = log_info_t[self.BASE_INFO] if self.BASE_INFO in log_info_t else json.loads("{}")
    ...
    def _add_app_name(self, tljs_obj, all_in_one_json_log):
        """
        ...
        """
        tljs_obj[self.APP_NAME] = all_in_one_json_log[self.APP_NAME]
        pass

    def _get_app_name(self, all_in_one_json_log):
        """
        ...
        for e in all_in_one_json_log[self.APP_NAME]: 
            if e[self.KEY] == self.UUID:
                return e[self.VALUE]

def main():
    converter = AppAllLogsSummaryConverter()
    converter.convert()
